# Deferred Spawn/Send Analysis

The core issue: `ctx.Spawn(...)` and `Send(ctx, ...)` look like they execute immediately, but they actually buffer requests that only take effect after the handler returns its `Suspend` value.

**Option A: Rename methods** (`DeferSpawn`, `BufferSend`)
...
**Option B: Fold into Suspend return value** — e.g. `Suspend(Select(...), Spawn(...), Send(...))`
- Consistent with the declarative model — everything the handler wants to happen is expressed in the return value.
- But fanout and pipeline examples call `Send`/`Spawn` in loops (variable number of calls), which doesn't fit a single return value cleanly. Would need a builder or variadic approach that's more awkward than imperative calls.
...
**Option D: Pending tokens** — return a value from `Spawn`/`Send` that is never used
- e.g. `_ = ctx.Spawn(...)` or `token := Send(ctx, ...)`
...
- Keep the current imperative `ctx.Spawn(...)` and `Send(ctx, ...)` signatures.
- Add prominent doc comments: "Buffers a spawn/send request. The request is executed by the runtime after the handler returns its Suspend value, not immediately."
